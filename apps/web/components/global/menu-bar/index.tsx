...
const MenuBar = ({ menuList, onSetSection, currentLabel }: Props) => {
  return (
    <Card className="p-0 border border-[#37372c] ">
      <CardContent className="gap-2 items-center p-1 bg-[#27272a] bg-clip-padding backdrop-filter backdrop-blur-2xl bg-opacity-60 flex rounded-xl">
        {menuList.map((menuItem, index) => (
          <div
            ...
            className={cn(
              "rounded-lg flex gap-2 py-1.5 px-4 item-center text-[12px] cursor-pointer transition-all duration-200 ease-in-out border border-transparent ",
