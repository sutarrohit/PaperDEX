...
COPY --from=builder --chown=nodejs:nodejs /app/packages/order-service/dist ./dist
COPY --from=builder --chown=nodejs:nodejs /app/packages/order-service/package.json .
